...
describe("POST /api/mail/pr", () => {
  ...
  it("returns 401 when authorization header is missing", async () => {
    ...
    const request = new NextRequest("http://localhost/api/mail/pr", {
      method: "POST",
  ...
  it("forwards request to upstream and mirrors response", async () => {
    ...
    const request = new NextRequest("http://localhost/api/mail/pr", {
      method: "POST",
      body: JSON.stringify({ project_id: "P-123" }),
      headers: {
    ...
    expect(fetchSpy).toHaveBeenCalledWith("https://api.appliedbas.com/v2/mail/pr", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer test-token",
