...
describe("POST /api/mail/sr", () => {
  ...
  it("returns 401 when authorization header is missing", async () => {
    ...
    const request = new NextRequest("http://localhost/api/mail/sr", {
      method: "POST",
  ...
  it("forwards request to upstream and mirrors response", async () => {
    ...
    const request = new NextRequest("http://localhost/api/mail/sr", {
      method: "POST",
      body: JSON.stringify({ report_no: 123 }),
      headers: {
    ...
    expect(fetchSpy).toHaveBeenCalledWith("https://api.appliedbas.com/v2/mail/sr", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer test-token",
