...
const UPSTREAM = "https://api.appliedbas.com/v2/mail/sr";

/** Same-origin proxy for service report mail send. */
export async function POST(request: NextRequest) {
  ...
  if (shouldSkipMailUpstream()) {
    console.info(
      "[api/mail/sr] Skipped upstream mail send (dev / SKIP_MAIL_ON_SUBMIT).",
    );
    return NextResponse.json(
      {
        message:
  ...
  try {
    ...
    console.error("SR mail proxy upstream fetch failed:", err);
    return NextResponse.json(
      {
        message:
